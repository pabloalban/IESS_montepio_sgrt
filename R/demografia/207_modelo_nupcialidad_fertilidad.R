...
fer_dat <- copy( cen_2010[ tipo == 'J' & tipo_dep %in% c( 'J', 'H' ) ] )
fer_dat[ , nj := 0 ]
fer_dat[ tipo_dep == 'J', nj := 1 ]
fer_dat[ , n := 0 ]
fer_dat[ tipo_dep == 'H', n := 1 ]
fer_dat <- fer_dat[ , list( n = sum( n ), nj = sum( nj ) ), by = list( sexo, x, y, sexo_dep ) ]
fer_dat[ , N := sum( nj ), by = list( sexo, x ) ]
fer_dat[ , q_obs := n / N ]
fer_dat <- fer_dat[ , list( sexo, x, sexo_dep, y, nj, n, q_obs ) ]
fer_dat[ , u_obs := log( -log( q_obs ) ) ]
setorder( fer_dat, x, y )

fer_dat_prep <- copy( fer_dat[ x <= 100 ] )
# fer_dat_prep[ x - y <= 15 & !is.na( q_obs ), q_obs := 0 ]
fer_dat_prep[ , u_obs := log( -log( q_obs ) ) ]
fer_dat_prep <- fer_dat_prep[ is.finite( u_obs ) ]
# fer_dat_prep[ , uq := quantile( u_obs, 0.95 ), by = list( sexo ) ]
# fer_dat_prep <- fer_dat_prep[ u_obs <= uq ]
fer_dat_prep[ , var := 0.3 * var( sqrt( abs( x - y ) ) * u_obs ), b = list( x ) ]
fer_dat_prep <- fer_dat_prep[ !is.na( var ) ]
# fer_dat_prep[ , varx := mean( var, na.rm = TRUE ) ]
# fer_dat_prep[ is.na( var ), var := varx ]
# fer_dat_prep[ , varx := NULL ]

# summary( fer_dat_prep$var )
# chk <- fer_dat_prep[ , list(
#    q_obs = sum( q_obs, na.rm = TRUE ),
#    u_obs = mean( u_obs, na.rm = TRUE ) ),
#    by = list( x ) ]
# plot( chk$x, chk$q_obs, pch = 16, cex = 0.8, col = "darkgreen" )
# plot( chk$x, chk$u_obs, pch = 16, cex = 0.8, col = "darkgreen" )
# plot( chk$x, chk$var, pch = 16, cex = 0.8, col = "darkgreen", ylim = c( 0, 20 ) )
# boxplot( u_obs ~ x, data = fer_dat_prep )
...
fer_dat_hh[ , sexo := 'H' ]
fer_dat_hh[ , sexo_dep := 'H' ]

# chk <- fer_dat_hh[ , list(
#    u_obs = mean( u_obs, na.rm = TRUE ),
#    q_obs = sum( q_obs, na.rm = TRUE ),
#    u = mean( u, na.rm = TRUE ),
#    q = sum( q, na.rm = TRUE ) ), by = list( x ) ]
# plot( chk$x, chk$u, type = 'l', lwd = 2, col = "navyblue", ylim = c( 0, 20 ) )
# points( chk$x, chk$u_obs, pch = 16, cex = 0.8, col = "darkgreen" )
# 
# plot( chk$x, chk$q, type = 'l', lwd = 2, col = "navyblue", ylim = c( 0, 2 ) )
# points( chk$x, chk$q_obs, pch = 16, cex = 0.8, col = "darkgreen" )
# 
# hist( fer_dat_hh$r / sd( fer_dat_hh$r, na.rm = TRUE ), probability = TRUE )
# curve( dnorm, add = TRUE )
# 
# fer_surf <- dcast.data.table( data = fer_dat_hh, formula = x ~ y, value.var = 'u' )
# fer_surf[ , x := NULL ]
# fer_surf <- as.matrix( fer_surf )
# fer_point <- fer_dat_hh[ !is.na( u_obs ) & is.finite( u_obs ) ]
# persp3d( x, y, fer_surf, col = 'gold', alpha = 0.7, xlim = range( x ), ylim = range( y ), zlim = c( -1, 5 ) )
# points3d( fer_point$x, fer_point$y, fer_point$u_obs, col = 'red', add = TRUE, zlim = c( -1, 5 ) )
...
fer_krig_mm <- km( formula = ~1, 
                   ...
                   covtype = 'matern5_2',
                   optim.method = 'BFGS', 
                   noise.var = 1.2 * fer_dat_prep_mm$var )
