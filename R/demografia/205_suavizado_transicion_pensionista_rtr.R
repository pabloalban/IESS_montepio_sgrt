...
tas_2_12 <- expand.grid( anio = year_grid,
  ...
  left_join( ., ERx_act, by = c( 'anio', 'sexo', 'x' ) ) %>% 
  group_by( x, sexo ) %>%
  mutate( ERx_act_2022 = sum( ifelse( anio == '2022',
                                      ERx_act,
                                      0 ), na.rm = TRUE ) ) %>%
  ungroup( ) %>% 
  group_by( sexo ) %>%
  mutate( Nx_ing_2022 = sum( ifelse( anio == '2022',
                                     Nx_ing,
                                     0 ), na.rm = TRUE ) ) %>% 
  ungroup( ) %>% 
  group_by( x, sexo ) %>% 
...
# Hombres 545 ingresos 2012 a 2022, y solo 79 ingresos de mujeres, total 624
# Ingresos de 21 en 2022, 20 hombres y una mujer
...
tas_2_12_h <- expand.grid( sexo = c( 'H' ),
  ...
  left_join( ., pred, by = 'x' )
  #%>% 
  # mutate( t_2_12_int = ( t_2_12_int * Nx_ing_2022 /  sum ( t_2_12_int * ERx_act_2022, na.rm = TRUE ) ) ) %>% 
...
#Comprobación: 21 ingresos de pensionistas en 2022
sum ( tas_2_12_h$t_2_12_int * tas_2_12_h$ERx_act_2022, na.rm = TRUE )
...
tas_12_6 <- expand.grid( anio = year_grid,
  ...
  left_join( ., sgrt_pen_tran_pa_pt_pp_anio, by = c( 'anio', 'sexo', 'x' ) ) %>%
  group_by( x ) %>% 
  mutate( ERx_incp_2022 = sum( if_else( anio == '2022',
                                      ERx_incap,
                                      0 ), na.rm = TRUE ),
          Nx_sal_2022 = sum( if_else( anio == '2022',
                                    Nx_sal,
                                    0 ), na.rm = TRUE ) ) %>% 
  ungroup( ) %>% 
  group_by( x ) %>% 
...
#Total 920 salidas entre 2012 a 2022, 170 salidas en 2022 
# Hombres 840 salidas entre 2012 a 2022 y 161 en 2022
# Mujeres 80 salidas entre 2012 a 2022 y 9 en 2022
...
tas_12_6 <- expand.grid( #sexo = c( 'H' ),
  ...
  left_join( ., pred, by = 'x' )  %>% 
  mutate( t_12_6_int = ( t_12_6_int * sum( Nx_sal_2022, na.rm = TRUE ) /  sum ( t_12_6_int * ERx_incp_2022, na.rm = TRUE ) ) ) %>% 
  mutate( t_12_6 = ( t_12_6 * sum( Nx_sal_2022, na.rm = TRUE ) /  sum ( t_12_6 * ERx_incp_2022, na.rm = TRUE ) ) ) %>% 
  dplyr::select( -sexo )
...
#Comprobación:
sum ( tas_12_6$t_12_6_int * tas_12_6$ERx_incp_2022, na.rm = TRUE )
...
tas_2_13 <- expand.grid( anio = year_grid,
  ...
  left_join( ., ERx_act, by = c( 'anio', 'sexo', 'x' ) ) %>% 
  group_by( sexo, x ) %>% 
  mutate( ERx_act_2022 = sum( if_else( anio == 2022,
                                       ERx_act,
                                       0 ), na.rm = TRUE ) ) %>% 
  ungroup( ) %>% 
  group_by( sexo ) %>% 
  mutate( Nx_ing_2022 = sum( if_else( anio == 2022,
                                    Nx_ing,
                                    0 ), na.rm = TRUE ) ) %>%  
  ungroup( ) %>% 
  group_by( sexo, x ) %>% 
...
# Hombres, 5061 ingresos entre 2012 a 2022, en 2022 hay 303 ingresos
tas_2_13_h <- tas_2_13 %>% 
  filter( sexo == 'H' )
...
tas_2_13_h <- left_join( tas_2_13_h, pred, by = 'x' ) %>% 
  mutate( t_2_13 = ( t_2_13 * Nx_ing_2022 /  sum ( t_2_13 * ERx_act_2022, na.rm = TRUE ) ) ) %>% 
  mutate( t_2_13_int = ( t_2_13_int * Nx_ing_2022 /  sum ( t_2_13_int * ERx_act_2022, na.rm = TRUE ) ) )
...
#Comprobación: 
sum ( tas_2_13_h$t_2_13_int * tas_2_13_h$ERx_act_2022, na.rm = TRUE )

# Mujeres hay  901 ingresos entre 2012 a 2022 y en 40 ingresos en 2022
tas_2_13_m <- tas_2_13 %>% 
  filter( sexo == 'M' )
...
#Comprobación: 
sum ( tas_2_13_m$t_2_13_int * tas_2_13_m$ERx_act_2022, na.rm = TRUE )
...
# Hombres hay 208.627 subsidios pagados entre 2012 y 2022; y 11.442 subsidios pagados en 2022   
...
tas_2_14_h <- left_join( tas_2_14_h, pred, by = 'x' ) %>% 
  mutate( t_2_14 = ( t_2_14 * Nx_ing_2022 /  sum ( t_2_14 * ERx_act_2022, na.rm = TRUE ) ) ) %>% 
  mutate( t_2_14_int = ( t_2_14_int * Nx_ing_2022 /  sum ( t_2_14_int * ERx_act_2022, na.rm = TRUE ) ) )
...
#Comprobación: 
sum( tas_2_14_h$t_2_14_int * tas_2_14_h$ERx_act_2022, na.rm = TRUE )

# Mujeres 43.135 subsidios entre 2012 a 2022, en 2022 hay 2.400
...
#Comprobación: 
sum( tas_2_14_m$t_2_14_int * tas_2_14_m$ERx_act_2022, na.rm = TRUE )
...
tas_0_15 <- expand.grid( anio = year_grid,
  ...
  group_by( sexo, x ) %>%
  mutate( ERx_hij_2022 = sum( ifelse( anio == '2022',
                                      ERx_hij,
                                      0 ), na.rm = TRUE ) ) %>%
  ungroup( ) %>% 
  group_by( sexo ) %>% 
...
#Hombres 1308 ingresos entre 2012 a 2022, 35 ingresos en 2022 
tas_0_15_h <- tas_0_15 %>% 
  filter( sexo == 'H' ) 
...
#%>% 
 # mutate( t_0_15 = ( t_0_15 * Nx_ing_2022 /  sum ( t_0_15 * ERx_hij_2022, na.rm = TRUE ) ) ) %>% 
  # mutate( t_0_15_int = ( t_0_15_int * Nx_ing_2022 /  sum ( t_0_15_int * ERx_hij_2022, na.rm = TRUE ) ) )
...
#Comprobación:
sum ( tas_0_15_h$t_0_15_int * tas_0_15_h$ERx_hij_2022, na.rm = TRUE )

#Mujeres 1231 ingresos entre 2012 a 2022, 33 ingresos en 2022
tas_0_15_m <- tas_0_15 %>% 
  filter( sexo == 'M' ) 
...
#Comprobación:
sum ( tas_0_15_m$t_0_15_int * tas_0_15_m$ERx_hij_2022, na.rm = TRUE )
...
tas_15_0 <- expand.grid( anio = year_grid,
  ...
  #filter( x <= 21 ) %>% 
  group_by( sexo ) %>% 
  mutate( Nx_sal_2022 = sum( if_else( anio == '2022',
                                      Nx_sal,
                                      0 ), na.rm = TRUE ) ) %>%
  ungroup( ) %>% 
  group_by( sexo, x ) %>% 
  mutate( ERx_incap_2022 = sum( if_else( anio == 2022,
                                         ERx_incap,
                                         0 ), na.rm = TRUE ) ) %>% 
  ungroup( ) %>% 
  group_by( sexo, x ) %>% 
...
# Hombres Salidas de 1117 hombres, 115 salidas en 2022
tas_15_0_h <- tas_15_0 %>% 
  filter( sexo == 'H' ) 
...
tas_15_0_h <- expand.grid( sexo = c( 'H' ),
  ...
  mutate( t_15_0_int = if_else( x %in% c( '18', '21' ),
                                t_15_0,
                                t_15_0_int ) ) %>% 
  mutate( t_15_0 = ( t_15_0 * Nx_sal_2022 /  sum ( t_15_0 * ERx_incap_2022, na.rm = TRUE ) ) ) %>% 
  mutate( t_15_0_int = ( t_15_0_int * Nx_sal_2022 /  sum ( t_15_0_int * ERx_incap_2022, na.rm = TRUE ) ) )

#Comprobación:
sum( tas_15_0_h$t_15_0_int * tas_15_0_h$ERx_incap_2022, na.rm = TRUE )
...
# Mujeres 1107 salidas entre 2012 a 2022, en 2022 hay 112 salidas
tas_15_0_m <- tas_15_0 %>% 
  filter( sexo == 'M' ) 
...
#Comprobación:
sum( tas_15_0_m$t_15_0_int * tas_15_0_m$ERx_incap_2022, na.rm = TRUE )
...
tas_0_16 <- expand.grid( anio = year_grid,
  ...
  group_by( x, sexo ) %>%
  mutate( ERx_cony_2022 = sum( ifelse( anio == '2022',
                                       ERx_cony,
                                      0 ), na.rm = TRUE ) ) %>%
  ungroup( ) %>% 
  group_by( sexo ) %>%
...
# Hombres solo hay 60 ingresos entre 2012 y 2022, 1 ingreso en 2022 
# Mujeres hay 1466 ingresos entre 2012 a 2022, 47 ingresos en 2022
...
tas_0_16_m <- expand.grid( sexo = c( 'M' ),
  ...
  mutate( t_0_16_int = ( t_0_16_int * Nx_ing_2022 /  sum ( t_0_16_int * ERx_cony_2022, na.rm = TRUE ) ) ) %>%
...
#Comprobación: 48 ingresos de pensionistas en 2022
sum ( tas_0_16_m$t_0_16_int * tas_0_16_m$ERx_cony_2022, na.rm = TRUE )
...
# Hombres 10 salidas entre 2012 a 2022, 2 en 2022
# Mujeres 602 salidas entre 2012 a 2022, 102 en 2022
...
tas_16_0 <- tas_16_0 %>% 
  ...
  mutate( Nx_sal = sum( Nx_sal, na.rm = TRUE ),
          ERx_incap = sum( ERx_incap, na.rm = TRUE ),
          Nx_sal_2022 = sum( Nx_sal_2022, na.rm = TRUE ),
          ERx_incap_2022= sum( ERx_incap_2022 , na.rm = TRUE ) ) %>% 
  ungroup( ) %>% 
  distinct( x, .keep_all = TRUE ) %>% 
  mutate( t_16_0 = Nx_sal / ERx_incap ) %>% 
  dplyr::select( -sexo )
...
tas_16_0 <- left_join( tas_16_0, pred, by = 'x' ) %>% 
  mutate( t_16_0 = ( t_16_0 * Nx_sal_2022 /  sum ( t_16_0 * ERx_incap_2022, na.rm = TRUE ) ) ) %>% 
  mutate( t_16_0_int = ( t_16_0_int * Nx_sal_2022 /  sum ( t_16_0_int * ERx_incap_2022, na.rm = TRUE ) ) )
...
#Comprobación:
sum ( tas_16_0$t_16_0_int * tas_16_0$ERx_incap_2022, na.rm = TRUE )
