...
PEAr <- merge.data.table( 
  ...
  all.x = TRUE )
PEAr[ , pr := 0 ]
PEAr[ ERx + ERxr > 0, pr := pmin( 4.1 * ERxr / ( ERx + ERxr ), 1 ) ]
...
# Corrección para el defecto del datos de activos que cumplen condiciones de eligibilidad
...
# Corrección para el defecto del datos de activos que cumplen condiciones de eligibilidad
l0xs[ , sct := mapply( FUN = parametros$elig_vej_ssc, s + 3, x + 3 ) - mapply( FUN = parametros$elig_vej_ssc, s - 3, x - 3 ) ]
l0xs[ sct < 0, sct := 0 ]
l0xs[ sct == 1, l2 := l2 + 0.8 * l3 ]
l0xs[ sct == 1, l3 := 0.2 * l3 ]
