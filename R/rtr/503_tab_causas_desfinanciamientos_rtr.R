...
aux <- as.data.table( causa_desf_estado %>% dplyr::select( -coeficiente ) )
...
# Tabla CD 501--------------------------------------------------------------------------------------
message( '\tTabla aportes del estado' )
aux <- as.data.table( causa_desf_CD501 %>% dplyr::select( -coeficiente ) )
aux[, tasa_rendimiento := tasa_rendimiento * 100 ]
...
aux <- ( causa_desf_desinversiones %>% filter( capital_desinvertido > 0) %>% dplyr::select( -factor ) )
